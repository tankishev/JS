...
const endpoints = {
    catalogue: `/data/games`,
    details: (reqIDs) => `/data/games/${reqIDs.itemId}`,
    login: `/users/login`,
    logout: '/users/logout',
    register: '/users/register',
    readAll: '/data/games?sortBy=_createdOn%20desc',
    readLatest: '/data/games?sortBy=_createdOn%20desc&distinct=category',
    // readItemsByUser: (reqIDs) => `/data/memes?where=_ownerId%3D%22${reqIDs.userId}%22&sortBy=_createdOn%20desc`
    bonusCatalog: '/data/comments',
    bonusReadItem: (reqIDs) => `/data/comments?where=gameId%3D%22${reqIDs.itemId}%22`
...
const requestHandlers = {
    ...
    readLatest: {method: 'get', endpoint: 'readLatest', authorize: false},
